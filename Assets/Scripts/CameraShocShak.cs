    ...
    public void AddForce(ForceModeEnum _forceModeEnum){
        ...
        if(!shakeMode.isRotate){
            if(shakeMode.shakeModeEnum == ShakeModeEnum.V)
                cameraMove.ShakeCamera(shakeMode.duration, shakeMode.strengthV, shakeMode.vibrato, shakeMode.randomness, shakeMode.snapping, shakeMode.fadeOut);
            else if(shakeMode.shakeModeEnum == ShakeModeEnum.F)
                cameraMove.ShakeCamera(shakeMode.duration, shakeMode.strengthF, shakeMode.vibrato, shakeMode.randomness, shakeMode.snapping, shakeMode.fadeOut);
        }else{
            if(shakeMode.shakeModeEnum == ShakeModeEnum.V)
                cameraMove.ShakeCameraRotation(shakeMode.duration, shakeMode.strengthV, shakeMode.vibrato, shakeMode.randomness, shakeMode.fadeOut);
            else if(shakeMode.shakeModeEnum == ShakeModeEnum.F)
                cameraMove.ShakeCameraRotation(shakeMode.duration, shakeMode.strengthF, shakeMode.vibrato, shakeMode.randomness, shakeMode.fadeOut);
        }
    }
